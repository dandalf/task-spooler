...
static void server_loop(int ls) {
    ...
    while (keep_loop) {
        ...
        for (i = 0; i < nconnections; ++i) {
            fds[npolls].fd = client_cs[i].socket;
            fds[npolls].events = POLLIN;
            ++npolls;
        }
        ...
        poll(fds, npolls, POLL_TIMEOUT);

        if (ls_index == nconnections
            && (fds[ls_index].revents & POLLIN) == POLLIN) {
            int cs;
            fds[ls_index].revents = 0;
            cs = accept(ls, NULL, NULL);
            if (cs == -1)
                error("Accepting from %i", ls);
            client_cs[nconnections].hasjob = 0;
            client_cs[nconnections].socket = cs;
            fds[nconnections].fd = cs;
            fds[nconnections].events = POLLIN;
            ++nconnections;
        }
